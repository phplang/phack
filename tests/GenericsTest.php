...
class PhackGenericsTest extends PHPUnit\Framework\TestCase {
    ...
    public function testClassGenerics() {
        ...
        $this->assertTranspiles(
            '/** * @template T */ class Foo { /** @var T */ public $data; }',
            'class Foo<T> { public T $data; }'
        );
        
        $this->assertTranspiles(
            '/** * @template A * @template B as Bar */ class Foo { }',
            'class Foo<A, B as Bar> {}'
        );
        
        $this->assertTranspiles(
            '/** * @template T */ class Foo { /** * @return T */ public function bar() { } }',
    ...
    public function testFunctionGenerics() {
        $this->assertTranspiles(
            '/** * @template Tk * @template Tv * @param Tk $key * @param Tv $val * @return Tv */ function foo($key, $val) { }',
            'function foo<Tk,Tv>(Tk $key, Tv $val): Tv {}'
        );

        $this->assertTranspiles(
            '/** * @template T as Foo * @param T $x */ function foo($x) { }',
            'function foo<T as Foo>(T $x) {}'
        );

        $this->assertTranspiles(
            '/** * @template T super Foo * @param T $x */ function foo($x) { }',
    ...
    public function testMethodGenerics() {
        $this->assertTranspiles(
            'class C { /** * @template Tk * @template Tv * @param Tk $key * @param Tv $val * @return Tv */ function foo($key, $val) { } }',
